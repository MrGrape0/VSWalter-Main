...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		//Thanks EpicGamer,helped me on the tween/backdrop code lol

		walter = new FlxSprite(500, 800);
		walter.frames = Paths.getSparrowAtlas('WalterTitle');
		walter.animation.addByPrefix('idle', 'Walter Coffee', 24);
		walter.antialiasing = ClientPrefs.globalAntialiasing;
		walter.animation.play('idle');

		backdrops = new FlxBackdrop(Paths.image('backdrop'));
		backdrops.velocity.set(150, 150);
		...
		backdrops.visible = false;
		add(backdrops);
		walter.visible = false;
		add(walter);

		logoBl = new FlxSprite(-50, 70);
		...
		demo = new FlxSprite(1500, 450);
		demo.frames = Paths.getSparrowAtlas('demo');
		demo.antialiasing = ClientPrefs.globalAntialiasing;
		demo.animation.addByPrefix('bump', 'Demo', 24);
		demo.animation.play('bump');
		demo.visible = false;
		demo.updateHitbox();
		add(demo);
		...
		walter.shader = swagShader.shader;
		logoBl.shader = swagShader.shader;
		demo.shader = swagShader.shader;
	...
	override function update(elapsed:Float)
	{
		...
		if (!transitioning && skippedIntro)
		{
			if(pressedEnter)
			{
				...
				FlxTween.tween(demo, { x: -800,}, 2, {ease: FlxEase.expoInOut});
				FlxTween.tween(logoBl, { x: -800 }, 2, {ease: FlxEase.expoInOut});
				FlxTween.tween(walter, { x: 1500, y: 50 }, 2, {ease: FlxEase.expoInOut});
				FlxTween.tween(titleText, { y: 1000 }, 2, {ease: FlxEase.expoInOut});
				//I like Tweens :-)
	...
	override function beatHit()
	{
		...
		if(walter != null) 
			walter.animation.play('Walter Coffee');
		...
		if(!closedState) {
			...
			{
				case 1:
					createCoolText(['Walter mod by'], 15);
				...
				// credTextShit.text += '\nNight';
				case 15:
					addMoreText('Walter'); // credTextShit.text += '\nFunkin';
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			FlxTween.tween(demo, { x: 180 }, 2, {ease: FlxEase.expoInOut});
			FlxTween.tween(logoBl, { y: -10 }, 1.5, {ease: FlxEase.expoInOut});
			FlxTween.tween(walter, { x: 600, y: 50 }, 1.5, {ease: FlxEase.expoInOut});
