...
class PlayState extends MusicBeatState
{
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['week1_nomiss', 'week2_nomiss', 'week3_nomiss', 'week4_nomiss',
				'week5_nomiss', 'week6_nomiss', 'week7_nomiss', 'bad',
				'ur_good', 'hype', 'two_keys', 'toastie', 'debugger', 'walter_nomiss', 'walter_end', 'dead']);
	...
	override function beatHit()
	{
		...
		if(Paths.formatToSongPath(SONG.song) == 'monochrome')
			{
				switch (curBeat)
				{ 
					case 28:
	...
	private function checkForAchievement(achievesToCheck:Array<String>):String {
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName)) {
				...
				{
					case 'week1_nomiss' | 'week2_nomiss' | 'week3_nomiss' | 'week4_nomiss' | 'week5_nomiss' | 'week6_nomiss' | 'week7_nomiss' | 'walter_nomiss':
						if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							...
							switch(weekName) //I know this is a lot of duplicated code, but it's easier readable and you can add weeks with different names than the achievement tag
							{
								...
								case 'museum':
									if(achievementName == 'walter_nomiss') unlock = true;
							}
						}
					case 'ur_bad':
					...
					case 'walter_end':
						if(Paths.formatToSongPath(SONG.song) == 'chill-ties' && !usedPractice) {
							unlock = true;
						}
					case 'dead':
						if(Paths.formatToSongPath(SONG.song) == 'monochrome' && songMisses < 1 ) {
							unlock = true;
						}
				}
